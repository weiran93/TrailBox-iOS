...
- `SessionStore`、`DeepLinkRouter`、`SavedRoutesStore`、`RecentRoutesStore` 在 app 入口注入为 environment object。
...
- `TrailBox/RecentRoutesStore.swift` 在本机保存最多 8 条公开路线轻量摘要，不保存 GPS 点；打开公开路线详情后会更新，探索页提供最近浏览入口与清除操作。
