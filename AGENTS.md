...
`BottomBarVisibilityStore` 当前定义在 `TrailBox/RootView.swift` 中，不存在独立的 `TrailBox/BottomBarVisibilityStore.swift` 构建阻塞。
...
| `RootView.swift` 中的 `BottomBarVisibilityStore` | 控制自定义底部导航栏显隐 |
